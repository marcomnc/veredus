    ...
    extends Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price_Configurable 
{
    ...
    protected function _applyConfigurableOption()
    {
        ...
        $groupRoundPriceExp= $write->getCheckSql("{$percenExpr} = 1", $roundExpr, $groupPrice);
        ...
        if ($this->getVersionHelper()->isGe1700()) {
           $seletColumnlist['group_price'] = $groupPriceColumn;
        }
        $select->columns($seletColumnlist);
        
        $query = $select->insertFromSelect($coaTable);
        $write->query($query);
        
        $seletColumnlist = array('parent_id', 
                                 'customer_group_id', 
                                 'website_id', 
                                 'MIN(price)', 
                                 'MAX(price)', 
                                 ...
                                 'store_id');
        if ($this->getVersionHelper()->isGe1700()) {
           $seletColumnlist[] = 'MIN(group_price)';
        }
        $select = $write->select()
            ->from(array($coaTable), $seletColumnlist)
        ...
        if ($this->getVersionHelper()->isGe1700()) {
            $seletColumnlist['group_price'] = $write->getCheckSql('i.group_price IS NOT NULL',
                                                                  'i.group_price + io.group_price', 'NULL'
                                                                    );
        }
        $select->columns($seletColumnlist);
